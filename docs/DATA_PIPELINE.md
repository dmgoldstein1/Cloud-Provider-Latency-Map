...
must work from `file://`. All data is inlined into `data/data.js` (and the world
map into `data/world.js`) by `scripts/build_data_js.py`. If you change any
`data/*.json`, re-run that script.
...
## 4. Dashboard data contract: `data/data.js` and `data/world.js`

`index.html` loads these scripts (all `defer`red except the async world map):

```
lib/d3.custom.min.js            <- custom d3 bundle (only the modules the app uses)
lib/topojson-client.min.js
data/data.js                    <- core data: regions + measured matrices (needed at boot)
data/world.js                   <- world topojson; <script async>, not needed to boot
js/app.min.js                   <- js/{config,normalize,map,charts,app}.js, bundled+minified
```

`js/app.min.js`, `lib/d3.custom.min.js`, and the inlined CSS in `index.html`
are **build outputs**: edit the sources (`js/*.js`, `css/style.css`) and run
`npm run build` (see `scripts/build_frontend.mjs`). The build also inlines the
minified stylesheet into `index.html` between the `INLINE-CSS` markers and
drops the external `<link>`. `npm run smoke` boots the page in headless Chrome
and checks all four charts render; `npm run perf` times a throttled load.

`data/data.js` and `data/world.js` are generated by `scripts/build_data_js.py`,
which inlines the JSON files into `window.VML_DATA`:

| property | file | Contents |
|---|---|---|
| `VML_DATA.measured` | `data/data.js` | **the measured metric data** |
| `VML_DATA.regions` | `data/data.js` | region metadata: code, name, lat/lon, continent |
| `VML_DATA.world` | `data/world.js` | world topojson for the map (async; the `land` object is pruned, only `objects.countries` is used) |

The world map is split out of the boot path because it is ~108KB and the app
only needs it once the map's landmass renders; `data/world.js` dispatches a
`vml-world` event that `js/app.js` listens for to draw the land when it
arrives. Rebuild after touching any of the JSON files:

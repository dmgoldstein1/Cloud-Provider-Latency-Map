...
`index.html` loads these scripts in order (all `defer`red):

```
lib/d3.custom.min.js            <- custom d3 bundle (only the modules the app uses)
lib/topojson-client.min.js, lib/d3-scale-chromatic.min.js
data/data.js                    <- ALL data lives here (globals, no fetch)
js/app.min.js                   <- js/{config,normalize,map,charts,app}.js, bundled+minified
```

`js/app.min.js`, `lib/d3.custom.min.js`, and `css/style.min.css` are **build
outputs**: edit the sources (`js/*.js`, `css/style.css`) and run `npm run build`
(see `scripts/build_frontend.mjs`). `npm run smoke` boots the page in headless
Chrome and checks all four charts render.

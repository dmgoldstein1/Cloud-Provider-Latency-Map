(function () {
  ...
  function renderScatter() {
    ...
    // On phones the left margin is trimmed to the minimum that keeps the
    // rotated y-axis label fully visible (~15px, it extends left of its anchor)
    // plus the tick numbers (~27px + d3's 9px text offset): padL 56 with the
    // label anchored at x=15. Any tighter and the label hangs past the SVG's
    // left edge and gets clipped at the viewport.
    var padL = mobile() ? 56 : 104, padR = 14, padT = 10, padB = 52;
    ...
    yLabel.join('text')
      ...
      .attr('transform', 'translate(' + (mobile() ? 15 : 33) + ',' + (padT + innerH / 2) + ') rotate(-90)')
      .attr('text-anchor', 'middle')
